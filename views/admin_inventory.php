...
if ($action === 'edit' && $roomId > 0) {
    ...
    if (!$roomCheck) { setFlash('error', 'Kamar tidak ditemukan.'); redirect('?page=admin_inventory'); }
}
?><?php require __DIR__ . '/../includes/layout.php'; ?>
<?php
if ($action === 'create' || ($action === 'edit' && $roomId > 0)):
    $room = ['name' => '', 'description' => '', 'facilities' => '', 'price_per_month' => '', 'status' => 'Available', 'image_path' => null];
    if ($action === 'edit') {
        $room = $roomCheck;
    }
    $errors = $persistedErrors;
    // Merge persisted data over existing room data for re-display
...
?>
    <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            <form method="POST" action="?page=admin_rooms&action=<?= $action ?><?= $action === 'edit' ? '&id=' . $roomId : '' ?>" enctype="multipart/form-data" class="space-y-5">
                <div>
                ...
                <div class="flex gap-3">
                    ...
                    <a href="?page=admin_inventory" class="border border-gray-300 text-gray-700 font-medium px-6 py-2.5 rounded-lg hover:bg-gray-50 transition text-sm">Batal</a>
                </div>
            </form>
        </div>
    </div>
...
?>
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div class="flex items-center justify-between mb-6">
            <h1 class="text-2xl font-bold text-gray-900">Kelola Kamar</h1>
            <a href="?page=admin_inventory&action=create"
        ...
        <?php if (empty($rooms)): ?>
            <div class="text-center py-16 bg-white rounded-xl border border-gray-200">
                ...
                <a href="?page=admin_inventory&action=create" class="inline-block bg-brand text-white px-6 py-2.5 rounded-lg hover:bg-emerald-600 transition text-sm">Tambah Kamar Pertama</a>
            </div>
        <?php else: ?>
                        ...
                        <tr class="hover:bg-gray-50 transition">
                            ...
                            <td class="px-4 py-3">
                                <div class="flex items-center gap-2">
                                    <a href="?page=admin_inventory&action=edit&id=<?= (int) $room['id'] ?>" class="text-brand hover:text-emerald-600 font-medium text-xs">Edit</a>
                                    <a href="?page=admin_rooms&action=delete&id=<?= (int) $room['id'] ?>"
                                       onclick="return confirm('Hapus kamar ini?')"
                                       class="text-red-500 hover:text-red-600 font-medium text-xs">Hapus</a>
                                </div>
                            </td>
                        </tr>
                        <?php endforeach; ?>
