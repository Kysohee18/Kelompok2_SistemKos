...
$sql = $queryMap[$status] ?? $queryMap['waiting'];
$stmt = $pdo->query($sql);
...
<div class="flex flex-wrap gap-1 mb-6 border-b border-gray-200">
    ...
    <?php foreach ($tabs as $key => $label): ?>
        <a href="?page=admin_dashboard&tab=transactions&status=<?= $key ?>"
           class="px-5 py-2.5 text-sm font-medium rounded-t-lg transition
                  <?= $status === $key ? 'bg-white text-brand border-l border-t border-r border-gray-200 -mb-px' : 'text-gray-500 hover:text-gray-700 hover:bg-gray-100' ?>">
            <?= $label ?>
        </a>
    <?php endforeach; ?>
</div>
                ...
                <div class="flex flex-col items-end gap-2 flex-shrink-0">
                    <?php if ($tx['status'] === 'WAITING_VALIDATION'): ?>
                        ...
                        <div class="flex gap-2">
                            <form method="POST" action="?page=admin_process&action=approve_tx&id=<?= (int) $tx['id'] ?>">
                            ...
                            <form method="POST" action="?page=admin_process&action=reject_tx&id=<?= (int) $tx['id'] ?>">
